...
def _stub_signing(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_sign_payload(payload: dict[str, str], client_id: str, client_secret: str, **_: Any):
        return {**payload, "client_id": client_id, "nonce": "n0nce", "signature": "sig"}

    monkeypatch.setattr(sdk_users, "sign_payload", fake_sign_payload)


def _post(monkeypatch: pytest.MonkeyPatch, envelope: dict[str, Any], captured: dict[str, Any] | None = None):
...
class TestCreateSdkUserSuccess:
    ...
    def test_carries_a_nonce_and_signature_and_never_the_secret(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        captured: dict[str, Any] = {}
        _post(monkeypatch, OK_ENVELOPE, captured)
        create_sdk_user(**VALID)
        assert captured["data"]["nonce"] and captured["data"]["signature"]
    ...
    def test_omits_optional_fields_rather_than_sending_them_empty(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        captured: dict[str, Any] = {}
        _post(monkeypatch, OK_ENVELOPE, captured)
...
class TestCreateSdkUserFailure:
    def test_non_zero_status_raises_even_though_http_is_200(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
    ...
    def test_status_zero_with_no_code_is_a_contract_change_not_a_success(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
    ...
    def test_http_error_is_wrapped_and_the_body_is_not_leaked(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        def fake_post(url: str, **_: Any):
    ...
    def test_accepts_a_valid_three_character_alphanumeric_shortname(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
    ...
    def test_rejects_an_out_of_range_gender(
        self, monkeypatch: pytest.MonkeyPatch, bad_gender: int
    ) -> None:
        _post(monkeypatch, OK_ENVELOPE)
    ...
    def test_rejects_an_out_of_range_mailingpref(
        self, monkeypatch: pytest.MonkeyPatch, bad_pref: int
    ) -> None:
        _post(monkeypatch, OK_ENVELOPE)
