...
description: "Data types by provider: Apple, Garmin, Polar, Suunto, Oura, Whoop, Ultrahuman, Strava, Fitbit, Google Health Connect, and Withings. OAuth, workouts, sleep, and timeseries support."
keywords: ["wearable api coverage", "health data types by provider", "wearable data types", "provider data support", "Withings API"]
"og:title": "Wearable API Coverage Matrix | Open Wearables"
"og:description": "Health data types by provider: Apple, Garmin, Polar, Suunto, Oura, Whoop, Ultrahuman, Strava, Fitbit, Google Health Connect, and Withings."
"og:url": "https://docs.openwearables.io/providers/coverage"
"og:type": "article"
"twitter:title": "Wearable API Coverage Matrix | Open Wearables"
"twitter:description": "Health data types by provider: Apple, Garmin, Polar, Suunto, Oura, Whoop, Ultrahuman, Strava, Fitbit, Google Health Connect, and Withings."
...
| Capability | Apple | Garmin | Polar | Suunto | Oura | Whoop | Ultrahuman | Strava | Fitbit | Health Connect | Withings |
|------------|:-----:|:------:|:-----:|:------:|:----:|:-----:|:----------:|:------:|:------:|:--------------:|:--------:|
| **OAuth** | ❌ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ❌ | ✅ |
| **Workouts** | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ❌ | ✅ | ✅ | ✅ | ✅ |
| **Sleep** | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ❌ | 🔜 | ✅ | ✅ |
...
<Accordion title="Withings">
  **Integration Method:** OAuth 2.0 + Webhooks (rest_pull + webhook_ping)
  ...
  - Body measures from `getmeas`: weight, height, lean body mass, body fat %, body fat mass, blood pressure (systolic/diastolic), heart rate, body temperature, skin temperature, oxygen saturation, skeletal muscle mass, bone mass, body water mass, blood glucose, VO2 max, and vascular age mapped to `cardiovascular_age`; plus the provider-specific `withings_pulse_wave_velocity` and `withings_metabolic_age` series
  - Deferred `getmeas` types are listed with their reason in the `DEFERRED_MEASURE_TYPES` map in `withings/coverage.py` — generic temperature type 12 needs device-aware environmental-versus-body classification; extracellular/intracellular water compartments (kg, not the core mL intake series), AFib classifications (not burden), segmental composition, ECG intervals, nerve health scores, nerve/skin conductance, visceral fat, and BMR (a rate, not basal energy expenditure) all need explicit core-model agreement before ingestion
  - Daily activity from `getactivity`: steps, distance, and active calories are stored as provider-native daily totals; the passive component of total calories is derived as `totalcalories - calories` and stored as the daily `basal_energy` total, avoiding active-energy double counting
  ...
  - Daily activity records marked `brand = 18` are externally sourced echoes and are skipped; missing `deviceid` alone is not an origin signal
  - Workouts with category `128` (No activity) are skipped; unknown categories are retained as `other` and logged once per category
  - Withings API uses an RPC-over-POST convention with an `action` parameter and a `{status, body}` envelope — envelope unwrapping, status handling, and pagination are centralised in `withings/_client.py`, and the equivalent token-envelope handling in `withings/oauth.py`
  - Shared provider infrastructure was extended for this convention, additively and backwards-compatibly: `make_authenticated_request` gained `form_data` for form-encoded POST bodies plus an `acquire_slot` hook (used here to draw from the application-wide Withings request budget); `ProviderCapabilities` gained a `webhook_subscription_per_user` flag alongside the existing `webhook_registration_api` and `BaseProviderStrategy` an `effective_live_sync_mode` helper plus an `on_disconnect` teardown hook; `BaseWebhookService` gained a `reconcile_user_subscriptions` entry point for per-user providers
  - Two of those changes are cross-provider by construction: the unified webhook route now answers HEAD (`probe_provider_webhook`) for callback-reachability probes on every provider's path, and `user_connection_service.is_last_active_provider_link` gates OAuth deregistration and subscription teardown for all providers, not only Withings
