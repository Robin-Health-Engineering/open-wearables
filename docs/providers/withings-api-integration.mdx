...
# Withings' application-wide limit. Only change this if Withings grants you a higher quota.
WITHINGS_API_REQUESTS_PER_MINUTE=120
```
    ...
    **Configuration details:**
    ...
    | `WITHINGS_API_REQUESTS_PER_MINUTE` | Withings' application-wide rate limit. Open Wearables paces every Withings API call against this value; leave it at the default unless Withings has granted your app a higher tier |
...
| `user.sleepevents` | Bed-sensor event notifications (bed in/out, inflate completion) - **requested by default, but not yet consumed**: Open Wearables does not subscribe to appli 50/51/52. It is requested because OAuth scope is fixed at authorization time, so adding it later would force every already-connected member to re-authorize |
...
Withings enforces this limit once per registered application, not per user - calling an endpoint for two users counts as two requests against the same budget. Open Wearables paces every Withings API call against `WITHINGS_API_REQUESTS_PER_MINUTE` automatically, so you shouldn't need to throttle requests yourself unless Withings has granted your app a higher tier.
