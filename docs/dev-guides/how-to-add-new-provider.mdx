...
| `webhook_subscription_owner` | Who owns the provider's webhook subscriptions: `APPLICATION` for one app-level registration covering every user, `USER` for one subscription per connection created with that user's own bearer token. `None` (the default) means they cannot be registered programmatically and must be configured by hand in the provider's developer portal. | `APPLICATION`: Oura, Polar, Strava, Google. `USER`: Withings |
| `webhook_inbound_secret` | Provider signs inbound payloads with a secret returned by the registration call. Requires `webhook_subscription_owner` to be set. | Polar |
...
1. Set `webhook_subscription_owner=WebhookSubscriptionOwner.APPLICATION` in your strategy's `ProviderCapabilities`.
...
<Note>
If `webhook_subscription_owner` is unset (the default), switching sync mode to `webhook` only updates the database — you must register subscriptions manually in the provider's developer portal pointing to `{API_BASE_URL}/api/v1/providers/{provider}/webhooks`.
</Note>

#### Per-user subscriptions

Some providers create subscriptions with each user's own bearer token rather than one app-level registration (e.g. Withings). For these, set `webhook_subscription_owner=WebhookSubscriptionOwner.USER` instead of `APPLICATION`, and implement `sync_user(db, user_id, mode)` (and optionally `remove_user`) on your `BaseWebhookService` rather than `register_subscriptions`. The platform fans this out per active connection — on connect, on a live-sync mode toggle, and on a daily reconciliation sweep — instead of calling it once for the whole application.
...
- [ ] If `webhook_subscription_owner=APPLICATION`: wired `webhook_service` on the strategy and implemented `register_subscriptions()` on it, and verified auto-registration fires on sync mode switch
- [ ] If `webhook_subscription_owner=USER`: implemented `BaseWebhookService.sync_user()`/`remove_user()` and verified the fan-out reconciles one subscription per active connection
